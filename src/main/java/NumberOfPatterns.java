...
public class NumberOfPatterns {
    public static void main(String[] args) {
        Assert.assertEquals(numberOfPatterns(1, 2), 65);
        Assert.assertEquals(numberOfPatterns(1, 1), 9);
    }
    ...
    static int helper(int num, int len, int res, int m, int n, int[][] jumps, boolean[] visited) {
        if (len >= m) ++res;
        ++len;
        if (len > n) return res;
        visited[num] = true;
